...
export default class LineGraphTide extends React.Component {
  ...
  render() {
    ...
    let count = 0
    let dataArr = []
    if (tideArr === undefined) {
      return (
        <Spinner className="spinner" name="double-bounce" color="white" />
      );
    } else {
    ...
    const lineData = [{ x: 1, y: 4 }, { x: 2, y: 4 }, { x: 3, y: 4 }, { x: 4, y: 4 }, { x: 5, y: 4 }, { x: 6, y: 4 }, { x: 7, y: 4 }];

    return <div className="tideChartMedia"><XYPlot
      width={350} height={150} yDomain={[-2, 6]}>
      <YAxis style={{ color: "white" }} title="Tide FT" tickValues={[-2, 0, 2, 4, 6]} />
